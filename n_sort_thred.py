...
def sort_files(location_sort_folder: Path, location_new_folder: Path):
    files = [x for x in location_sort_folder.iterdir() if x.is_file()]
    # print(files)
    folders = [x for x in location_sort_folder.iterdir() if x.is_dir()]
    # print(folders)

    for element in files:#location_sort_folder.iterdir():
    ...
    for element in folders:#location_sort_folder.iterdir():
        # if element.is_dir():
        #print(f"is dir = {element}")
        # sort_files(element, location_new_folder)
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            future_to_folder = {executor.submit(sort_files, element, location_new_folder): element for element in folders}
            for future in concurrent.futures.as_completed(future_to_folder):
                folder = future_to_folder[future]
                try:
                    future.result()
                except Exception as e:
                    print(f"Error moving files in folder {folder}: {e}")
